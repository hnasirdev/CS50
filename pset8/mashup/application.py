...
def articles():
   ...
   #  rows = db.execute("SELECT * FROM places")
    ...
    #lOOK UP NEWS FOR THAT GET LOCATION IN OUR URL AND RETURN IT AS A JSON ITEM
    if(len(articles)) >5:
        return jsonify([articles[0], articles[1], articles[2], articles[3], articles[4]])

    return jsonify(articles)

   # check = lookup(request.args.get("geo"))
   # return jsonify(check)
    # TODO
    #return jsonify(rows[place_name])


@app.route("/search")
def search():
    """Search for places that match query"""
    #somehow find something in databse that matches the form search using typehead?
